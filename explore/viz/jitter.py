...
def jitter(x, how='order',
           yprops=(.2, .5), ylim=None,
           jitter_cdf_width=0.05,
           color='black',
           s=1,
           zorder=1,
    ...
    for k in kwargs.keys():  # update defaults with user provided
        scatter_kws[k] = kwargs[k]

    y = jitter_yvals(x, how=how, yprops=yprops, ylim=ylim,
                     jitter_cdf_width=jitter_cdf_width)

    plt.scatter(x, y, **scatter_kws)


def jitter_yvals(values, how='order',
                 yprops=(.2, .5), ylim=None,
                 jitter_cdf_width=0.05):
    """
    ...
    """
    # TODO: make this not plot when we do plt.gca
    ...
    if how == 'cdf':
        ...
        yvals += np.random.uniform(low=-jitter_cdf_width,
                                   high=jitter_cdf_width,
                                   size=len(yvals))
